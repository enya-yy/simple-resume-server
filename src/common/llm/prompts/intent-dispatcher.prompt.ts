...
# 表单类意图的 responseText（CREATE_RESUME / EDIT_BASIC_INFO / ADD_EXPERIENCE）
- 卡片上方只显示**一句**简短引导（建议 15～35 字），口语自然即可
- 不要写「请填写表单」、不要列表、不要复述手机号邮箱
...
- 若事件表明**某一模块已成功保存**（文案含「模块已保存成功」或等价表述，如基本信息、工作经历、教育经历、项目经验、技能等）：结合【当前简历状态】用 2～4 句简短回复——先确认刚保存的模块，再概括简历整体进度（还缺哪些模块、不必逐条展开）。**不要**写「猜你想做」小标题，不要用「·」列举操作建议（界面底部已有同名快捷按钮，勿重复）。全文约 60～100 字；可用一句收尾提示「点击下方快捷按钮继续」，但不要复述按钮上的文案。
  ...
  parameters: {
    ...
    properties: {
      ...
      responseText: {
        type: 'string',
        description:
          '回复文本；表单类意图时写一句简短引导（15～35 字），展示在填写卡片上方',
      },
    },
  },
};
