...
function loadResumePreviewCss(): string {
  return readFileSync(join(__dirname, 'resume-preview.css'), 'utf8');
}
...
export function buildResumeExportHtml(doc: ResumeDocument): string {
      ...
      const items = section.items
        .map((item) => {
          const bullets = item.bullets
            ...
            .join('');
          return `<li>
            <p class="resume-preview-item-title">${escapeHtml(
              item.title || '（未命名条目）',
            )}</p>
        ...
        .join('');
      return `<li>
        <p class="resume-preview-module-title">${escapeHtml(
          section.title,
        )}（${escapeHtml(section.type)}）</p>
  ...
  const basicsBlock = hasBasics
    ? `<header class="resume-preview-basics" aria-label="基础信息">
      ${
        basics.fullName.trim()
          ? `<h3 class="resume-preview-basics-name">${escapeHtml(
              basics.fullName,
            )}</h3>`
          : ''
      }
      ${
        basics.headline.trim()
          ? `<p class="resume-preview-basics-headline">${escapeHtml(
              basics.headline,
            )}</p>`
          : ''
      }
      ${
        contactLine
          ? `<p class="resume-preview-basics-contact">${escapeHtml(
              contactLine,
            )}</p>`
          : ''
      }
      ${
        basics.summary.trim()
          ? `<div class="resume-preview-basics-summary rp-md">${resumeMarkdownToSafeHtml(
              basics.summary,
            )}</div>`
          : ''
      }
