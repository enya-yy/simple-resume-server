/**
 * 阿里云百炼 DashScope — OpenAI 兼容 Chat Completions（服务端/Worker 专用）。
...
type DashScopeTopError = { code?: string; message?: string };
...
function parseDashScopeError(rawText: string): { code?: string; message?: string } {
  try {
    const j = JSON.parse(rawText) as OpenAiStyleError & DashScopeTopError;
...
function userHintForDashScopeFailure(
  status: number,
  apiCode?: string,
  apiMessage?: string,
): string {
  ...
  if (status === 429) {
    return "百炼侧限流，请稍后重试。若问题持续，请联系支持并附上 requestId。";
  }
  if (status >= 500) {
...
export async function completeDashScopeChat(params: {
  apiKey: string;
  model: string;
  baseUrl: string;
  systemPrompt: string;
  ...
  if (!res.ok) {
    const { code: apiCode, message: apiMessage } = parseDashScopeError(rawText);
    const hint = userHintForDashScopeFailure(res.status, apiCode, apiMessage);
  ...
  try {
    ...
    throw new DashScopeRequestError(
      ...
      "百炼返回内容无法解析，请稍后重试。若问题持续，请联系支持并附上 requestId。",
      res.status,
    );
  }
...
export function getDashScopeEnv(): {
  apiKey: string | undefined;
  model: string;
  baseUrl: string;
} {
  const resolved = resolveDashScopeEnv(process.env);
  return {
    apiKey: resolved.apiKey,
    model: resolved.model || DASHSCOPE_DEFAULT_MODEL,
    baseUrl: resolved.baseUrl || DASHSCOPE_DEFAULT_BASE_URL,
  };
}
